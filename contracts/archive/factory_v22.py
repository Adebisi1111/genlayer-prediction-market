...
class Factory(gl.Contract):
    ...
    # Pull-over-push payouts: user -> amount
    ...
    def claim(self, market_id: str) -> u256:
        ...
        # Pull-over-push: add to payouts mapping
        current = int(self.payouts.get(sender, u256(0)))
        self.payouts[sender] = u256(current + payout)
    ...
    def withdraw(self) -> u256:
        ...
        gl.get_contract_at(gl.message.sender_address).emit_transfer(value=u256(amount), on='accepted')
        return u256(amount)

    @gl.public.view
    def getPayout(self, user: str) -> str:
        return str(int(self.payouts.get(user, u256(0))))
