...
// Try with empty args first to see if it's an arg parsing issue
try {
  const hash = await client.writeContract({ address: FACTORY, functionName: 'createMarket', args: ['Test market'], value: '0' });
  console.log(`Created: ${hash}`);
} catch (e) {
  console.log(`Error: ${e.message?.slice(0, 300)}`);
  console.log(`Cause: ${e.cause?.message?.slice(0, 200)}`);
}
